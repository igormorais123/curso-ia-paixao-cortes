# O PROTOCOLO — Ativacao do Advogado Aumentado
## Paixao Cortes Advogados | 4 horas (2h + 2h) | Hibrido | 3 cidades + online

> **A PREMISSA** (30 segundos, antes de tudo):
> "Voces ja sao advogados de elite. Tradicao, especializacao, Tribunais Superiores, 3 cidades operando.
> O que falta nao e competencia — e velocidade. Este protocolo nao ensina direito.
> Ele ATIVA o que voces ja sabem fazer, na velocidade que o mercado exige agora."

---

## A Jornada em 7 Fases

| Fase | Codigo | Frase de abertura | O que ativa |
|------|--------|-------------------|-------------|
| 1 | **BOOT** | "Primeira ativacao. Vejam a maquina operar." | A percepcao — "isso e real" |
| 2 | **SCANNER** | "Visao aumentada. Voces vao enxergar o que antes era invisivel." | A pesquisa — buscar com metodo |
| 3 | **ARSENAL** | "Carreguem o arsenal. Cada ferramenta e uma arma." | As ferramentas — qual usar quando |
| 4 | **FIREWALL** | "Todo sistema precisa de protecao. Este e o escudo." | A protecao — nao ser sancionado |
| 5 | **TESTE DE CAMPO** | "Caso real. Mostrem o que ativaram." | A execucao — fluxo completo |
| 6 | **ANALYTICS** | "Dados onde outros veem caos. O nivel estrategico." | A jurimetria — decidir com dados |
| 7 | **ATIVACAO COMPLETA** | "A partir de segunda-feira, voces sao diferentes." | O compromisso — ir ao campo |

### Slide de abertura (antes do BOOT)

> "Paixao Cortes. 3 cidades. Milhares de causas ganhas. Tudo isso, no braco.
> Imaginem tudo isso — com o protocolo ativado."

### Materiais na narrativa

| Material | Na narrativa | Quando entra |
|----------|-------------|--------------|
| 12 cartas CLEAR | **Protocolos de Ativacao** — cada carta e um protocolo que voce dominou | Fase 3 (ARSENAL) |
| Caso ficticio | **Simulacao** — cenario de teste controlado antes do campo real | Fase 5 (TESTE DE CAMPO) |

### Visual sugerido para slides
- **Palette**: Vinho do escritorio (#66142E) + dourado/tech-blue para ativacao
- **Transicoes**: Tela escura com nome da fase + frase de abertura (3 segundos)
- **Tipografia**: Sans-serif limpa (Apple, nao Cyberpunk)
- **Cards**: Verso com padrao de circuito minimalista — elegante, nao exagerado

---

## Dados da Execucao

| | Dia 1 | Dia 2 |
|---|---|---|
| **Data** | Quinta, 06/02/2026 | Sexta, 07/02/2026 |
| **Horario** | 15h - 17h | 10h - 12h |
| **Duracao liquida** | 120 min | 55 min caso + 40 min jurimetria + 25 min encerramento |
| **Formato** | Hibrido (3 cidades + online) | Hibrido (3 cidades + online) |

---

## Adaptacoes para Formato Hibrido

### Para o instrutor
- **Demo**: compartilhar tela o tempo todo. Camera ligada.
- **Exercicios**: cada participante faz no proprio celular/notebook, nao depende de parceiro fisico.
- **Compartilhamento**: pedir relatos por chat E por voz. Online primeiro (senao ficam mudos).
- **Timer visivel**: manter cronometro compartilhado na tela durante exercicios.
- **Check-in**: a cada 20 min perguntar "Online, estao acompanhando? Duvidas no chat."

### Para participantes online
- Ter ChatGPT ou Claude aberto em outra aba/celular.
- QR codes funcionam no celular normalmente.
- Duvidas: digitar no chat, serao respondidas nos intervalos entre blocos.
- Exercicios: mesmos que presencial. Resultado pode ser compartilhado por chat.

### Para participantes presenciais
- Levar celular carregado (ou carregador).
- QR codes projetados na tela ou impressos na mesa.
- Exercicios individuais (nao depende de trio/dupla).

### Materiais a enviar ANTES (email/WhatsApp)
- [ ] Link: ChatGPT (https://chatgpt.com)
- [ ] Link: Claude (https://claude.ai)
- [ ] Link: 12 Cartas CLEAR (https://inteia.com.br/paixaocortes/pagina_12_cartas_v3.html)
- [ ] Link: QR Codes (https://inteia.com.br/paixaocortes/qrcodes_curso.html)
- [ ] Instrucao: "Crie conta gratuita no ChatGPT OU Claude antes do curso."

---

## Filosofia (mantida)

> Nao convencer, converter. Nao ensinar sobre IA, fazer experimentar IA.

## Mandamentos v4 (adaptados ao hibrido)

1. **Espanto primeiro** — Demo WOW antes de qualquer slide
2. **Verificacao muscular** — Participante descobre o erro, nao ouve sobre ele
3. **Todo mundo faz** — Online e presencial fazem o mesmo exercicio ao mesmo tempo
4. **Pratica e conteudo** — 65% mao na massa, 35% instrucao
5. **Corte radical** — Se nao cabe em 4h, nao entra
6. **Presente, nao futuro** — Mostrar 2026, nao prometer 2030
7. **Ninguem fica para tras** — Check-in a cada 20 min com online

---

## Metricas v4

| Metrica | v3 (6h) | v4 (4h) | Corte |
|---------|---------|---------|-------|
| Instrucao | 100 min | 85 min (35%) | -15% |
| Pratica | 200 min | 155 min (65%) | -23% |
| Slides | 20 | 12 | -40% |
| Exercicios | 7 | 7 (+1 mini) | 0 |
| Casos ficticios | 3 | 1 | -2 |

---

# DIA 1 — QUINTA-FEIRA, 06/02 (15h - 17h = 120 min)

## FASE 1: BOOT — Espanto + Primeiro Contato (30 min)
**[Tela escura] "Primeira ativacao. Vejam a maquina operar."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-3 | 15:00 | Abertura + Demo | Tela compartilhada: IA gerando contestacao trabalhista em tempo real. Zero fala. | Online ve a tela |
| 3-5 | 15:03 | Primeira frase | "90 segundos. Uma contestacao estruturada. Voces tem 200 na fila. Facam a conta." | |
| 5-7 | 15:05 | Contexto | "Isso e o presente. Voces vao fazer agora. Abram ChatGPT ou Claude." | Enviar links no chat |
| 7-25 | 15:07 | **EXERCICIO 1** | Cada um abre IA, descreve caso de defesa, recebe analise em 60s. INDIVIDUAL. | Timer na tela: 18 min |
| 25-30 | 15:25 | Compartilhamento | "O que surpreendeu? O que decepcionou?" **Online responde primeiro** (chat ou mic). | Pedir 2-3 online, 2-3 presencial |

**Slides: 2** (QR codes das ferramentas, pergunta de compartilhamento)

**NOTA INSTRUTOR**: Demo = contestacao tipica do escritorio (ex-funcionario de banco, horas extras). Algo que produzem centenas por ano.

---

## FASE 2: SCANNER — Pesquisa + Verificacao (30 min)
**[Tela escura] "Visao aumentada. Voces vao enxergar o que antes era invisivel."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-2 | 15:30 | Setup | "Agora vamos pesquisar jurisprudencia com IA. Funciona, mas tem metodo." | |
| 2-15 | 15:32 | **EXERCICIO 2** | Pesquisar "horas extras gerente bancario TST" usando Protocolo de Pesquisa. Pedir links. | Timer: 13 min |
| 15-22 | 15:45 | **EXERCICIO 3** | Clicar nos links. Quantos existem? Anotar % de acerto. | Timer: 7 min |
| 22-25 | 15:52 | Revelacao | "Quando busca na internet com link = funciona. Quando inventa de cabeca = falha." | |
| 25-28 | 15:55 | Framework | **"Busca → Link → Thinking → Checagem"** (projetar na tela) | Slide fica 3 min |
| 28-30 | 15:58 | Regulacao | CNJ 615/2025 + OAB 001/2024. Advogado americano sancionado. 1 slide rapido. | |

**Slides: 2** (protocolo 4 passos, regulacao)

### Protocolo de Pesquisa (projetar e manter visivel)
1. **BUSCAR NA INTERNET** — Ativar busca web sempre
2. **EXIGIR LINK** — Sem link = nao existe
3. **THINKING MODE** — Ativar raciocinio estendido
4. **AUTOCHECAGEM** — "Revise todas as citacoes. Confirme que cada link funciona."

---

## FASE 3: ARSENAL — Ferramentas + Pratica Comparativa (35 min)
**[Tela escura] "Carreguem o arsenal. Cada ferramenta e uma arma."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-8 | 16:00 | Mapa honesto | 5 ferramentas, forcas e fraquezas. Sem ilusoes. | Slide na tela |
| 8-28 | 16:08 | **EXERCICIO 4** | Mesmo caso de defesa no ChatGPT E no Claude. Comparar: qual da melhor contestacao? Qual pesquisa melhor? | Timer: 20 min |
| 28-35 | 16:28 | CLEAR + Cartas | Framework CLEAR em 3 min + mostrar 2 cartas. "Link das 12 cartas no chat." | Enviar link no chat |

**Slides: 2** (mapa ferramentas, CLEAR resumido)

### Mapa de Ferramentas (slide unico)

| Ferramenta | Forca | Usar para |
|------------|-------|-----------|
| **ChatGPT** (modo de raciocinio) | Deep Research, Thinking | Pesquisa com fontes, rascunhos em massa |
| **Claude** (Opus linha atual) | Extended Thinking, textos longos | Pareceres, analise de contratos |
| **Perplexity** | Fontes nativas | Jurisprudencia com citacao |
| **NotebookLM** | Documentos longos, 1M tokens | Processos volumosos |
| **Gemini** (Gemini 3) | Deep Research, multimodal, OCR | Documentos digitalizados, imagens, Google Drive |

---

## FASE 4: FIREWALL — Seguranca + Encerramento Dia 1 (25 min)
**[Tela escura] "Todo sistema precisa de protecao. Este e o escudo."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-5 | 16:35 | Casos reais | Advogado EUA sancionado. Vazamento. 1 slide, impacto rapido. | |
| 5-10 | 16:40 | 5 Regras | Protocolo inegociavel (projetar, ler junto). | Slide fica visivel |
| 10-20 | 16:45 | **EXERCICIO 5** (mini) | Anonimizar 1 trecho de caso real: trocar nomes, mascarar dados, enviar para IA. | Timer: 10 min |
| 20-25 | 16:55 | Encerramento Dia 1 | Tarefa: "Ate amanha, usem IA em algo real. Tragam a experiencia." | |

### Protocolo de Seguranca (5 Regras — projetar)
1. **Anonimizar SEMPRE** — Nomes ficticios, dados mascarados
2. **Verificar TODA citacao** — Link verificavel ou nao existe
3. **Revisar TUDO** — Output e rascunho, nao produto final
4. **Registrar uso** — Transparencia (CNJ 615/2025)
5. **Humano decide** — IA sugere, advogado assina

**Slides: 2** (caso + 5 regras)

---

## TIMING DIA 1

| Fase | Inicio | Fim | Duracao |
|------|--------|-----|---------|
| 1. BOOT | 15:00 | 15:30 | 30 min |
| 2. SCANNER | 15:30 | 16:00 | 30 min |
| 3. ARSENAL | 16:00 | 16:35 | 35 min |
| 4. FIREWALL | 16:35 | 17:00 | 25 min |
| **Total** | | | **120 min** |

**Exercicios Dia 1**: 5 (Primeiro prompt, Pesquisa, Verificacao links, Comparativo ChatGPT/Claude, Anonimizacao mini)
**Pratica**: ~78 min (65%)
**Instrucao**: ~42 min (35%)

---

# DIA 2 — SEXTA-FEIRA, 07/02 (10h - 12h = 120 min)

## FASE 5: TESTE DE CAMPO — Caso Completo (55 min)
**[Tela escura] "Caso real. Mostrem o que ativaram."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-10 | 10:00 | Aquecimento | "O que tentaram ontem? O que funcionou?" **Online primeiro**. Celebrar. | Chat + mic |
| 10-50 | 10:10 | **EXERCICIO 6** | Fluxo completo com CASO FICTICIO. Individual. 3 etapas. | Timer: 40 min |
| 50-55 | 10:50 | Debrief | "Onde a IA ajudou mais? Onde atrapalhou?" | 2-3 relatos |

### EXERCICIO 6: Fluxo Completo de Defesa (40 min)

**Caso**: Dispensa discriminatoria — reclamante alega discriminacao por idade apos retorno de licenca medica. Escritorio defende a empresa.

**3 etapas**:
1. **Pesquisa** (12 min): Usar IA para pesquisar jurisprudencia de defesa. Aplicar Protocolo (Busca → Link → Thinking → Checagem).
2. **Minuta** (18 min): Gerar contestacao aplicando CLEAR. Impugnar ponto a ponto.
3. **Verificacao** (10 min): Checar links, pedir autochecagem, revisar.

**Instrutor circula (presencial) e monitora chat (online).**

**Slides: 1** (caso ficticio projetado na tela)

---

## FASE 6: ANALYTICS — Jurimetria (40 min)
**[Tela escura] "Dados onde outros veem caos. O nivel estrategico."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-5 | 10:55 | Transicao | "Agora vamos para algo que muda a estrategia de contencioso de massa: jurimetria." | |
| 5-15 | 11:00 | O que e | Jurimetria = dados + estatistica aplicados ao direito. Exemplos concretos para defesa empresarial. | Slide com exemplos |
| 15-25 | 11:10 | Demo pratica | Mostrar: pedir a IA para analisar padroes de decisao de uma vara. "Qual o % de condenacao em horas extras na 3a Vara de Brasilia?" Discutir limitacoes. | Tela compartilhada |
| 25-27 | 11:20 | Aplicacoes | Para contencioso de massa: previsao de risco, provisao financeira, estrategia por vara/juiz, momento ideal de acordo. | Slide com casos |
| 27-35 | 11:22 | **EXERCICIO 7** (mini) | Cada um pergunta a IA: "Qual o perfil decisorio da [vara/juiz] em [tema do escritorio]?" Comparar resposta com experiencia real. | Timer: 8 min |
| 35-40 | 11:30 | Ferramentas | Jurimetrics (ABAJ), Jusbrasil Analytics, Turivius, JUIT. "IA ajuda na analise, mas os dados precisam ser de fonte confiavel." | Slide com links |

**Slides: 3** (conceito, demo, ferramentas)

### Pontos-chave de Jurimetria para Contencioso de Massa
- **Provisao**: estimar valor de risco por tipo de acao
- **Estrategia de acordo**: identificar quando acordo e mais barato que litigar
- **Perfil de juiz**: padroes decisorios por vara/magistrado
- **Alocacao de recursos**: priorizar casos com maior risco
- **Relatorio para cliente**: dados concretos, nao "achismo"

---

## FASE 7: ATIVACAO COMPLETA — Recursos + Compromisso (25 min)
**[Tela escura] "A partir de segunda-feira, voces sao diferentes."**

| Min | Hora | Atividade | Detalhe | Hibrido |
|-----|------|-----------|---------|---------|
| 0-8 | 11:35 | Recursos | Links para continuar: AB2L, Podcast Direito Digital, FGV gratuito. QR codes na tela. | Enviar links no chat |
| 8-15 | 11:43 | Compromisso | "Na segunda-feira, vou usar IA para _____." Escrever (papel ou chat). | Online: digitar no chat |
| 15-20 | 11:50 | Compartilhamento | 3-4 voluntarios (mix online + presencial). Celebrar. | |
| 20-25 | 11:55 | Encerramento | "Protocolo ativado. 3 cidades, uma equipe. A partir de segunda, voces operam diferente." Certificado. Foto. | Online: print de tela |

**Slides: 2** (recursos com QR, template compromisso)

### Recursos (projetar QR codes)
- **AB2L**: ab2l.org.br
- **Podcast Direito Digital**: Ana Frazao + Caitlin Mulholland (Spotify)
- **FGV Regulacao da IA**: Gratuito, 1h, 6.400+ alunos
- **CNJ 615/2025**: atos.cnj.jus.br/atos/detalhar/6001
- **OAB 001/2024**: Recomendacoes para IA na advocacia

---

## TIMING DIA 2

| Fase | Inicio | Fim | Duracao |
|------|--------|-----|---------|
| 5. TESTE DE CAMPO | 10:00 | 10:55 | 55 min |
| 6. ANALYTICS | 10:55 | 11:35 | 40 min |
| 7. ATIVACAO COMPLETA | 11:35 | 12:00 | 25 min |
| **Total** | | | **120 min** |

**Exercicios**: 1 completo (fluxo de defesa) + 1 mini (jurimetria)
**Pratica**: ~85 min (71%)
**Instrucao**: ~35 min (29%)

---

# CASO FICTICIO (unico — usado no Exercicio 6)

## Dispensa Discriminatoria — Defesa da Empresa

**Partes:**
- Reclamante: Maria Silva Santos, 52 anos, auxiliar administrativa
- Reclamada: Comercio ABC Ltda (escritorio defende a empresa)

**Fatos alegados pelo reclamante:**
Trabalhou de 15/03/2018 a 10/01/2026. Salario: R$ 2.800. Dispensada sem justa causa apos retorno de licenca medica de 30 dias (sindrome do panico). Empresa contratou funcionaria de 24 anos para mesma funcao, salario R$ 2.200. Alega que gerente disse "precisamos de sangue novo".

**Missao do participante (defesa):**
1. Pesquisar jurisprudencia favoravel a empresa em casos similares
2. Gerar contestacao impugnando: discriminacao, nexo causal, testemunho indireto
3. Teses de defesa possiveis: reestruturacao legitima, poder diretivo, ausencia de prova direta, reducao de quadro

**Documentos disponiveis:**
- Plano de reestruturacao da empresa (anterior a dispensa)
- Avaliacao de desempenho (regular)
- Comprovante de pagamento integral de verbas rescisorias
- Depoimento do gerente negando a frase

---

# O QUE FOI CORTADO (v3 → v4)

| Item | v3 | v4 | Motivo |
|------|----|----|--------|
| Exercicio 4 comparativo | 25 min | 20 min | Compactado |
| Exercicio 6 anonimizacao | 20 min | 10 min | Mini-exercicio |
| Exercicio 7 compromisso | 20 min escrito | 7 min | Simplificado |
| Bloco Horizontes | 15 min separado | Absorvido no encerramento | Nao cabe |
| Intervalo formal | 10 min cada dia | Removido | 2h nao precisa |
| Buffer | 40 min total | 0 | Sem espaco |
| Casos ficticios | 3 | 1 | Tempo |
| Baralho 27 cartas | Atividade separada | Removido | Formato hibrido inviavel |

---

# CHECKLIST PRE-CURSO (URGENTE — HOJE)

## Antes das 15h
- [ ] Testar link de videoconferencia (Zoom/Teams/Meet)
- [ ] Testar compartilhamento de tela
- [ ] Enviar links por email/WhatsApp para TODOS (online e presencial):
  - ChatGPT: https://chatgpt.com
  - Claude: https://claude.ai
  - 12 Cartas: https://inteia.com.br/paixaocortes/pagina_12_cartas_v3.html
  - QR Codes: https://inteia.com.br/paixaocortes/qrcodes_curso.html
- [ ] Instrucao: "Crie conta no ChatGPT ou Claude ANTES do curso"
- [ ] Preparar demo: contestacao trabalhista gerada em 90s
- [ ] Testar ChatGPT e Claude no celular (funciona no 4G?)
- [ ] Verificar se Wi-Fi do local funciona

## Para cada cidade presencial
- [ ] Projetor/TV funcionando
- [ ] Audio para quem esta presencial ouvir o instrutor
- [ ] Tomadas para carregar celular
- [ ] QR codes impressos OU projetados

## Para online
- [ ] Link da reuniao enviado
- [ ] Instrucao: "Ligue camera se possivel, use chat para duvidas"
- [ ] Moderador designado para monitorar chat? (ideal, nao obrigatorio)

---

*O PROTOCOLO v4 — Hibrido, 3 cidades + online, 4h (2+2) — 06/02/2026*
*Narrativa: "Ativacao do Advogado Aumentado" — conteudo tecnico intocado, moldura narrativa aplicada*
